...
- **`bmap(ip, bn)` 函数**: `kernel/fs.c` 中，将文件的逻辑块号 `bn` 转换为磁盘物理块号，是文件系统寻址的核心
...
**同步修改** `kernel/file.h` 中内存 inode `struct inode` 的 `addrs` 字段大小。

### 2. 修改 bmap：处理双重间接 (kernel/fs.c)
...
### 3. 修改 itrunc：递归释放双重间接块 (kernel/fs.c)
...
| kernel/file.h | 修改 | 同步修改内存 inode 的 `addrs` 大小 |
| kernel/fs.c | 修改 | `bmap` 添加双重间接处理，`itrunc` 添加双重间接释放 |
